...
/************************** Basic objects ******************************/
    ...
    Pd.objects['localbuffer'] = Pd.UniquelyNamedObject.extend({

        init: function(name) {
            this.setName(name);
            this.buffer = new Pd.arrayType(0);
        },

        // Don't need to copy the value because send~ doesn't have outlets.
        write: function(array) {
            this.buffer = array;
        },

        // Copy the values into `array`.
        read: function(array) {
            var i = 0,
                aLength = array.length,
                bLength = this.buffer.length,
                l = (aLength < bLength) ? aLength : bLength;

            // fill other values with zeros
            for (i = 0; i < l; i++) {
...
/************************** DSP one variable signal functions ******************************/

    Pd.objects['cos~'] = Pd.Object.extend({
...
/************************** DSP glue *****************************/
    ...
    // Base for [send~] and [receive~]
    var DSPSRBase = Pd.Object.extend({

        setLocalBufferName: function(name) {
            var oldName = this.name;
            this.name = name;
            if (this.patch) {
                var localbuf = Pd.getUniquelyNamedObject('localbuffer', name);
                if (!localbuf) throw (new Error('local buffer ' + name + ' doesn\'t exist'));
                this.localbuf = localbuf;
            }
            this.emit('change:localbufferName', oldName, name);
        },

        load: function() {
            this.setLocalBufferName(this.name);
        }
    });

    Pd.objects['send~'] = DSPSRBase.extend({
        ...
        init: function(name) {
            ...
            this.localbuf = null;
            this.dspTick = this.dspTickZeros;
            // Create the local buffer if it doesn't exist yet
            if (!Pd.getUniquelyNamedObject('localbuffer', name)) {
                new Pd.objects['localbuffer'](null, [name]);
            } else console.warning('local buffer ' + name + ' already exists.');
            this.on('inletConnect', this._onInletConnect);
            this.on('inletDisconnect', this._onInletDisconnect);
        },

        dspTickWriting: function() {
            this.localbuf.write(this.inlets[0].getBuffer());
        },

        dspTickZeros: function() {
            this.localbuf.write(Pd.fillWithZeros(Pd.newBuffer()));
        },
    ...
    // TODO: what does the inlet mean?? I can't access pdPedia...
    Pd.objects['receive~'] = DSPSRBase.extend({
        ...
        init: function(name) {
            this.name = name;
            this.localbuf = null;
            this.on('change:localbufferName', function() {
                //this.setDelay(this.delay);
            });
        },
